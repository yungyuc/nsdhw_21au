...
public:
    ...
    {
        if (!m_buffer.empty()) { std::vector<double, MyAllocator<double>>(alloc).swap(m_buffer); }
        const size_t nelement = nrow * ncol;
        if (nelement) { m_buffer.resize(nelement); }
        else          { m_buffer.clear(); }
        m_nrow = nrow;
        m_ncol = ncol;
    }
...
MyAllocator<double> Matrix::alloc = MyAllocator<double>();
...
size_t bytes() {
    return Matrix::alloc.counter.bytes();
}

size_t allocated() {
    return Matrix::alloc.counter.allocated();
}

size_t deallocated() {
    return Matrix::alloc.counter.deallocated();
}
...
PYBIND11_MODULE(_matrix, m) {
    ...
    py::class_<Matrix>(m, "Matrix")
        ...
        .def("elapsed", static_cast<double & (Matrix::*)()>(&Matrix::elapsed))
...
//     size_t base_alloc = allocated();
//     std::cout << "bytes: " << bytes() << std::endl;
//     std::cout << "allocated: " << allocated() << std::endl;
//     std::cout << "deallocated: " << deallocated() << std::endl;

//     Matrix res_mkl = multiply_mkl(mat1, mat2);
//     Matrix res_naive = multiply_naive(mat1, mat2);
...
//     std::cout << "allocated: " << allocated() << std::endl;
//     std::cout << "after multiplication: " << base_alloc + 2*8*size*size << std::endl;
//     // std::cout << "bytes: " << bytes() << std::endl;
//     // std::cout << "allocated: " << allocated() << std::endl;
//     // std::cout << "deallocated: " << deallocated() << std::endl;
//     return 0;
